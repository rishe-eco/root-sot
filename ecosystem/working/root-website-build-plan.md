...
**From:** _root
**Status:** Build plan — sequences three specs. **Thirteen of fourteen built; the Persian pass's §1 is done and §2 is the founder's. R3 is all that is left of the code.**
**Version:** 0.13 · 2026-08-14 · Owner: _root
...
> **~~V1b~~ · ~~F3~~ · ~~F2~~ · ~~V2~~ · ~~V3~~ · ~~V4~~ · ~~C0~~ · ~~R1~~ · ~~R2~~ · R3 · ~~C1~~ · ~~C2~~ · ~~R4~~ · the Persian pass (§1 ~~done~~, §2 open)**

**Struck through as of 2026-08-14.** Thirteen of fourteen are built. **Three have moved out of their planned slots** — C0 forward (§5b), C1 ahead of R3 (taking the escape hatch the previous revision of this paragraph offered), and now **R4 ahead of R3 as well**, on founder direction, for the same reason and with the same standing condition.

**Track C is complete. Track R is complete but for R3.** What remains is **R3 · the Persian pass**, and **R3's condition still stands** — it is placed *"deliberately after there are entries worth an ontology"* (§5). A tree over twelve entries is worse than a flat tag list. The corpus has still not grown, so R3 has now been passed over twice; that is the condition working, not the condition being ignored, but it is worth naming that **the only thing gating the last of track R is content, not code**.

**The Persian pass took its own turn, and split in half doing it.** Its file proposed a definition nothing had ever given it — §1 mechanical, §2 judgement — and that definition now stands: **§1 is done**, and §2 is not an engineer's to do. What §1 settled is worth carrying here rather than leaving in a repo README, because it is a rule about the product's own language: **digits are Persian, except identifiers** (`root-app` house rule 14), and the audit that applied it found the same font/digit mismatch in five separate places nobody had noticed.

**What the founder still owes this stage**, and nothing else can:

- **§2** — reading the Persian product end to end, *without* the English beside it. Reading them side by side checks accuracy; reading Persian alone is what tells you whether it sounds like a person. This is the acceptance criterion the whole pass exists for.
- **§3.2** — the style guide the pass produces (the numeral rule, the ZWNJ conventions, the terminology glossary §2 builds) belongs in **this canon**, hanging off Brand §5, not in `root-app` where it would die with the line numbers around it.
- **§1.2.1** — one open question the review added: **ordinals**. Step badges and, more consequentially, **the printed contract's article numbers** are still Latin figures inside Persian text. How a legal document numbers its own articles is a presentation decision, not a formatting preference.

**Which leaves R3 as the only unbuilt code**, on a condition that has now deferred it twice: the corpus is still small. Nothing is blocked on engineering.
...
### R4 · The agent — **built and verified, 2026-08-13** *(`30a27e9`)* — **taken ahead of R3**
...
**Built, and the boundary is structural.** The plan's sentence above is a *rights* rule; the build's contribution was recognising it cannot be a *prompt* rule. You cannot instruct a model not to quote text you have handed it — it fails under paraphrase, under a direct question, and under anything the paper itself says. So a `LINK_ONLY` entry's full text never enters a request, and there is nothing to quote. Three independent layers hold it: one gate function, a block builder that re-derives that gate rather than trusting its caller, and R1's own CHECK constraint already making "LINK_ONLY with a hosted file" an impossible row. **Rate limiting also stopped being deferrable here** — R2 deferred it and C2 noted it again; R4 is the first endpoint where an anonymous request spends money, so it has an Nginx zone of its own, a per-IP bucket, and a daily ceiling that fails closed. See [`R4.md`](../../../root-app/docs/development/R4.md).
...
### C2 · Comments, threads and the corpus admin — **built and verified, 2026-08-12** *(`26440ff`)*
...
**Built.** The anchor decision was the stage: C1 stored markdown source, a reviewer selects rendered text, and those are different strings. Offsets count into the **rendered** text — safe only because C1 pinned the renderers and froze the blocks — with the selected quote stored beside them and re-checked on every read. A mismatch renders the thread **detached**, never re-found; a comment highlighting the wrong sentence is worse than one highlighting nothing. "Reviewers do not see each other's comments" was built as an **ownership edge, not a capability** — one where-clause that also happens to be the entire permission check for replying and resolving. **Revoke refuses rather than works around** the non-empty-roles constraint when reviewer is an account's only role. See [`C2.md`](../../../root-app/docs/development/C2.md).
...
- **0.13 · 2026-08-14** — **The Persian pass's mechanical half is built**, and the pass's own two-part definition — proposed by its file because nothing had ever specified it — is now settled fact: §1 is the engineer's and is done; §2 is the founder's and is not. **The numeral rule is decided and written down** (`root-app` house rule 14): counts, totals, dates and money in Persian digits; years, DOIs, refs, versions, hashes and concept keys Latin. Applying it found the *same* bug five times — a correctly Persian-formatted number wrapped in a class forcing the Latin figure font onto it — plus two i18next templates printing raw JS numbers, because `{{count}}` selects a plural form and does not localize digits. The general locale key-parity test that had never existed now exists and found **zero** missing keys, which is the discipline holding rather than the test being useless. The `lang`/`dir` gap 0.12 recorded is closed as one component. **Two things were decided rather than fixed**: names render in the ambient direction (no language field exists to key off, and a script heuristic tuned for prose is not one), and **ordinals are still open** — the printed contract numbers its articles in Latin figures, which is the founder's call and is now recorded as §1.2.1. One review finding, fixed: a fifth ZWNJ inconsistency the hand-read missed, «به دست» beside «به‌دست» in the identical construction, now held by a test that tolerates the legitimate verb/adjective pairs rather than flattening them. **R3 is the only unbuilt code left, and it waits on corpus size, not on engineering. Everything else outstanding is §2, and it is Root's.**
- **0.12 · 2026-08-13** — **C2 and R4 are built, and both tracks C and R are closed but for R3**, which has now been passed over a second time on the same unchanged condition: the corpus is still small. R4 came ahead of R3 on founder direction. What each build settled is recorded in `root-app`'s README; what matters here is what they settled about *this file*. **C2 corrected a C0 promise §5b has carried since 0.1** — "bilingual templates … for invite, reset, reviewer-invite, new-comment and contract-revised" was never true: C0 built two and said so in its own header comment, but the promise text was never corrected. C2 wrote two more; **`contract-revised` remains unbuilt and is now recorded as promised-and-not-delivered** rather than quietly dropped. **R4 found that "reuse R2's query" could not be taken literally** — a `contains` filter needs its whole argument as one substring, so a natural-language question matched nothing at all; the integration suite caught it, not review. Two review findings, both fixed: C2's Review Room was mouse-only (the same class as the standing `test.fail()` below), and R4 could end a stream with no terminal event, leaving a reader on a spinner indefinitely. Three things were deliberately *not* fixed and are written down in `root-app/docs/development/later-tracks.md` and `persian-pass.md` §1.6.1 instead — the spend counter under-reports on failure, `ASK_MAX_TOKENS` shares its budget with adaptive thinking and has never been seen at real length, and **entry titles carry no `lang`/`dir` outside the reader**, which is the R2 review's bug surviving in the two places nobody looked. **The Persian pass is the default next; R3 waits on content, not code.**
