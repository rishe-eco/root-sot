...
**Version 1.0 · 2026-07-14 · Owner: _root**

---

## 1. Thesis

**The person authors their own life — in search of beauty. Root never authors it for them.**

Everything Root builds, refuses, prices, and says derives from this sentence. If a stranger knows only the name and this thesis, they should be able to predict our decisions. Beauty is the direction of the authorship, not a promise of outcomes: a repaired relationship, a peace, a well-taught mind, a tended life — all of it is the same search.

The deeper register of Beauty is intentional and stays uncommented in public materials.
...
We are not positioning against anyone. Root is not entering the self-improvement market; it's making its own game — a self-authorship company. Where it aims to grow: coaching, therapy, communication, peacebuilding, education. "In search of beauty" is the only frame that holds all five without instrumentalizing any of them.
...
Register test for any asset: set "in search of beauty" in it. If it reads philosophy, ship it. If it reads shampoo, redo it.

## 7. Names, taglines, architecture

**Master brand:** Root / ریشه. (Persian side revised from بن — coupon reading — per decision log.)
**Tagline:** **Root — in search of beauty · ریشه — در جست‌وجوی زیبایی** *(provisional pending the salon field test; see §9).*
**Two-tier rule:** the tagline is a compass, never a map — it must always appear with an orienting descriptor nearby (working draft: «پلتفرمی برای شناختن و ساختنِ خود» / "a platform for knowing and growing yourself"). Never show the tagline naked.
...
- **Salon test** on the tagline: show name + tagline cold, ask "what does this company do?" Mis-categorization (beauty/cosmetics), not dislike, is the failure metric. Both languages.
- Descriptor's final wording, both languages.
